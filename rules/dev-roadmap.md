...
| done | v0.1.4 reliability release | serialized viewer decode/same-folder fast path plus `CLIPBRD_E_CANT_OPEN` recovery; Release build + 18/18 tests + installed contention probe |
| done | v0.1.5 image clipboard compatibility release | completion popup, viewer, and pixel selection share the WinForms-compatible bitmap clipboard path; 19/19 tests + installed single-click probes |
| done | Settings custom image-extension registration | user-entered image extensions are validated, stored, registered as Windows app candidates, and included in the viewer/browser index; Debug build + 21/21 tests pass |
| done | Viewer incremental library refresh | new captures in the active folder update the sorted list directly, and F5 preserves the folder tree; Debug build + 23/23 tests pass |
| done | v0.1.6 file-association activation release | an existing file delivered through a Windows file association is dispatched to the open viewer even outside the indexed extension list; Release build + 24/24 tests |
| follow_up | Window-exclusion rule selection and settings-surface clipping repair | implementation and automated checks pass: retain executable path plus process name, choose the rule only by radio toggle, and constrain both WPF settings surfaces to their viewport; actual UI confirmation remains `NEEDS_USER_UI_CHECK` |
