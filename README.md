...
1. [최신 릴리스](https://github.com/ai-blink/Mini-Capture/releases/latest)에서 `MiniCapture-v0.1.5-win-x64-portable.zip`을 내려받습니다.
...
- 캡처 결과를 PNG로 자동 저장
- PNG, JPG, JPEG 이미지를 열고 설정에 등록한 GIF를 원본 애니메이션으로 재생하는 내장 뷰어
...
캡처 결과에서 뷰어를 열거나 PNG/JPG/JPEG 파일 또는 설정에 등록한 GIF를 `MiniCapture.exe`로 열 수 있습니다. GIF는 첫 프레임에 멈추지 않고 원본 애니메이션으로 재생됩니다. 뷰어 상단에서 파일 이동, 맞춤, 원본 크기, 확대/축소, 회전과 마크업 도구를 사용할 수 있습니다.
...
dotnet publish .\src\MiniCapture\MiniCapture.csproj `
  ...
  -p:DebugType=None `
  -o .\artifacts\publish\MiniCapture-v0.1.5-win-x64-portable
```
